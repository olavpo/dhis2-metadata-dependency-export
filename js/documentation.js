...
function makeReferenceList(basePath, metaData) {
	...
	if (metaData.trackedEntityTypes && metaData.trackedEntityTypes.length > 0) {
		...
		for (var i = 0; i < metaData.trackedEntityTypes.length; i++) {
			...
			tab.push([tet.name, (tet.lastUpdated ? tet.lastUpdated.substr(0, 10) : ""), tet.id]);
		}
	...
	if (metaData.trackedEntityAttributes && metaData.trackedEntityAttributes.length > 0) {
		...
		for (var tea of metaData.trackedEntityAttributes) {
			tab.push([tea.name, (tea.code ? tea.code : ""), (tea.description ? tea.description : ""), (tea.lastUpdated ? tea.lastUpdated.substr(0, 10) : ""), tea.id]);
		}
	...
	if (metaData.programs && metaData.programs.length > 0 && metaData.programStages && metaData.programStages.length > 0) {
		...
		let xlProgTab = [["Name", "Description", "Tracked Entity Type", "Last updated", "UID", "enrollmentDateLabel", "incidentDateLabel"]];
		let xlStageTab = [["Program Stage", "UID", "Last updated", "Program UID", "executionDateLabel", "dueDateLabel"]];
		...
		for (var h = 0; h < metaData.programs.length; h++) {
			...
			xlProgTab.push([prog.name, (prog.description ? prog.description : ""), (prog.programType != "WITHOUT_REGISTRATION" ? getName(prog.trackedEntityType.id, metaData) : ""), (prog.lastUpdated ? prog.lastUpdated.substr(0, 10) : ""), prog.id, (prog.enrollmentDateLabel ? prog.enrollmentDateLabel : ""), (prog.incidentDateLabel ? prog.incidentDateLabel : "")]);
			...
			for (var i = 0; i < prog.programStages.length; i++) {
				...
				tab.push([ps.name, (ps.lastUpdated ? ps.lastUpdated.substr(0, 10) : ""), ps.id]);
				xlStageTab.push([ps.name, ps.id, (ps.lastUpdated ? ps.lastUpdated.substr(0, 10) : ""), prog.id, (prog.executionDateLabel ? prog.executionDateLabel : ""), (prog.dueDateLabel ? prog.dueDateLabel : "")]);
			}
			...
			for (var ps of prog.programStages) {
				...
				for (var psde of ps.programStageDataElements) {
					...
					if (metaData.programStageSections && metaData.programStageSections.length > 0) {
						psSection = programSectionFromStageAndElement(ps.id, psde.dataElement.id, metaData);
					}
					let deName = getName(psde.dataElement.id, metaData);
					tab.push([ps.name, psSection, deName]);
					xlSectTab.push([ps.name, (psSection ? psSection : ""), deName]);
				}
			}
			content += utils.htmlTableFromArray(tab, true);
			...
			//Program rules
			if (metaData.programRules && metaData.programRules.length > 0) {
				...
				for (var pr of programRules) {
					tab.push([pr.name, (pr.description ? pr.description : ""), (pr.lastUpdated ? pr.lastUpdated.substr(0, 10) : ""), pr.id]);
					xlProgRuleTab.push([pr.id, pr.name, (pr.description ? pr.description : ""), pr.condition, prog.id]);
				}
				content += utils.htmlTableFromArray(tab, true);
			}

			//Program Rule Actions
			if (metaData.programRuleActions && metaData.programRuleActions.length > 0) {
				let pra;
				for (let i = 0; i < metaData.programRuleActions.length; i++) {
					pra = metaData.programRuleActions[i];
					xlProgRuleActionTab.push([pra.id, pra.programRule.id, pra.programRuleActionType, (pra.data ? pra.data : ""), (pra.content ? pra.content : "")]);
				}
			}
			...
			for (var ptea of prog.programTrackedEntityAttributes) {
				...
				tab.push([getName(ptea.trackedEntityAttribute.id, metaData), (ptea.lastUpdated ? ptea.lastUpdated.substr(0, 10) : ""), ptea.id, ptea.trackedEntityAttribute.id]);
				xlPteaTab.push([ptea.id, getName(ptea.trackedEntityAttribute.id, metaData), ptea.trackedEntityAttribute.id, (ptea.lastUpdated ? ptea.lastUpdated.substr(0, 10) : ""), prog.id]);
			}
			content += utils.htmlTableFromArray(tab, true);
		}
		...
		if (xlProgRuleTab.length > 1) {
			utils.appendWorksheet(utils.sheetFromTable(xlProgRuleTab, true), wrkBook, "programRules");
		}
		if (xlProgRuleActionTab.length > 1) {
			utils.appendWorksheet(utils.sheetFromTable(xlProgRuleActionTab, true), wrkBook, "programRuleActions");
		}
		if (xlProgRuleVariableTab.length > 1) {
			utils.appendWorksheet(utils.sheetFromTable(xlProgRuleVariableTab, true), wrkBook, "programRuleVariables");
		}
	...
	if (metaData.programIndicators && metaData.programIndicators.length > 0) {
		...
		tab = [["UID", "name", "shortName", "code", "description", "analyticsType", "expression", "filter", "decimals", "program UID", "programIndicatorGroupUIDs"]];

		var ind, type;
		for (var i = 0; i < metaData.programIndicators.length; i++) {
			let programId = "";
			ind = metaData.programIndicators[i];
			if (ind.program && ind.program.id) {
	...
	if (metaData.dataElements && metaData.dataElements.length > 0) {
		...
		tab = [["UID", "name", "shortName", "code", "description", "domainType", "valueType", "zeroIsSignificant", "optionSet.id", "categoryCombo Name", "Form Name"]];

		for (var i = 0; i < metaData.dataElements.length; i++) {
			de = metaData.dataElements[i];
			var comboName = "";
			if (de.categoryCombo && de.categoryCombo.id) {
				getName(de.categoryCombo.id, metaData);
			};
			tab.push([de.id, de.name, de.shortName, (de.code ? de.code : ""), (de.description ? de.description : ""), de.domainType, de.valueType, de.zeroIsSignificant, (de.optionSet ? de.optionSet.id : ""), comboName, (de.formName ? de.formName : "")]);
		}
	...
	if (metaData.dataElementGroups && metaData.dataElementGroups.length > 0) {
		...
		for (var item of metaData.dataElementGroups) {
			tab.push([item.name, (item.shortName ? item.shortName : ""), (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), item.id]);
		}
		content += utils.htmlTableFromArray(tab, true);
	...
	if (metaData.categoryCombos && metaData.categoryCombos.length > 0) {
		...
		tab = [["UID", "Name", "Last updated", "Code", "Categories", "dataDimensionType"]];
		...
		for (var i = 0; i < metaData.categoryCombos.length; i++) {
			...
			tab.push([cc.id, cc.name, (cc.lastUpdated ? cc.lastUpdated.substr(0, 10) : ""), (cc.code ? cc.code : ""), (elements.length > 0 ? elements.join("; ") : " "), (cc.dataDimensionType ? cc.dataDimensionType : "")]);
		}
	...
	if (metaData.categories && metaData.categories.length > 0) {
		...
		tab = [["UID", "Code", "Name", "shortName", "Category options", "dataDimensionType", "Last updated"]];
		...
		for (var i = 0; i < metaData.categories.length; i++) {
			...
			tab.push([ dec.id, (dec.code ? dec.code : ""), dec.name, (dec.shortName ? dec.shortName : ""), (elements.length > 0 ? elements.join("; ") : " "), (dec.lastUpdated ? dec.lastUpdated.substr(0, 10) : ""), (dec.dataDimensionType ? dec.dataDimensionType : "") ]);
		}
	...
	if (metaData.categoryOptions && metaData.categoryOptions.length > 0) {
		...
		for (var i = 0; i < metaData.categoryOptions.length; i++) {
			...
			tab.push([co.name, (co.lastUpdated ? co.lastUpdated.substr(0, 10) : ""), co.id]);
		}
	...
	if (metaData.categoryOptionCombos && metaData.categoryOptionCombos.length > 0) {
		...
		for (var i = 0; i < metaData.categoryOptionCombos.length; i++) {
			...
			tab.push([coc.name, (coc.lastUpdated ? coc.lastUpdated.substr(0, 10) : ""), coc.id]);
		}
	...
	if (metaData.categoryOptionGroupSets && metaData.categoryOptionGroupSets.length > 0) {
		...
		for (var i = 0; i < metaData.categoryOptionGroupSets.length; i++) {
			...
			tab.push([cogs.name, (cogs.lastUpdated ? cogs.lastUpdated.substr(0, 10) : ""), cogs.id]);
		}
	...
	if (metaData.categoryOptionGroups && metaData.categoryOptionGroups.length > 0) {
		...
		for (var j = 0; metaData.categoryOptionGroups && j < metaData.categoryOptionGroups.length; j++) {
			...
			tab.push([item.name, item.shortName, (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), item.id]);
		}
	...
	if (metaData.optionGroups && metaData.optionGroups.length > 0) {
		...
		for (let i = 0; i < metaData.optionGroups.length; i++) {
			...
			tab.push([optionGroup.name, (optionGroup.lastUpdated ? optionGroup.lastUpdated.substr(0, 10) : ""), optionGroup.id, (optionSetName ? optionSetName : ""), optionText])
		}
	...
	if (metaData.optionSets && metaData.optionSets.length > 0) {
		...
		tab = [["UID", "Code", "Name", "Last updated", , "Options"]];
		...
		for (var i = 0; i < metaData.optionSets.length; i++) {
			...
			tab.push([os.id, (os.code ? os.code : ""), os.name, (os.lastUpdated ? os.lastUpdated.substr(0, 10) : ""), elementText]);
		}
	...
	if (metaData.options && metaData.options.length > 0) {
		...
		let tabTemp = [["UID", "Code", "Name", "Last updated", "Option set UID"]];
		...
		for (var i = 0; i < metaData.options.length; i++) {
			...
			content += "<tr><td>" + optSetName + "</td><td>" + opt.name + "</td><td>" + opt.code + "</td><td>" + (opt.lastUpdated ? opt.lastUpdated.substr(0, 10) : "") + "</td><td>" + opt.id + "</td></tr>";
			tabTemp.push([opt.id, opt.code, opt.name, (opt.lastUpdated ? opt.lastUpdated.substr(0, 10) : ""), optSetId]);
		}
		content += "</table>";
	...
	if (metaData.validationRules && metaData.validationRules.length > 0) {
		...
		tab = [["UID", "Name", "shortName", "code", "Instruction", "periodType", "Left side description", "Left side expression", "Operator", "Right side description", "Right side expression"]];

		for (var i = 0; i < metaData.validationRules.length; i++) {
			var vr = metaData.validationRules[i];

			tab.push([vr.id, vr.name, (vr.shortName ? vr.shortName : ""), (vr.code ? vr.code : ""), (vr.instruction ? vr.instruction : ""), vr.periodType, vr.leftSide.description, vr.leftSide.expression, vr.operator, vr.rightSide.description, vr.rightSide.expression]);
		}
	...
	if (metaData.validationRuleGroups && metaData.validationRuleGroups.length > 0) {
		...
		for (var j = 0; metaData.validationRuleGroups && j < metaData.validationRuleGroups.length; j++) {
			...
			tab.push([item.name, (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), item.id]);
		}
	...
	if (metaData.predictorGroups && metaData.predictorGroups.length > 0) {
		...
		for (let i = 0; i < metaData.predictorGroups.length; i++) {
			...
			tab.push([predictorGroup.name, (predictorGroup.lastUpdated ? predictorGroup.lastUpdated.substr(0, 10) : ""), predictorGroup.id, predictorText])
		}
	...
	if (metaData.predictors && metaData.predictors.length > 0) {
		...
		tab = [["UID", "Name", "code", "periodType", "missingValueStrategy", "generator.description", "generator.expression", "Output data element", "outputCombo", "Sequential samples", "Annual samples"]];
		...
		for (var i = 0; i < metaData.predictors.length; i++) {
			...
			tab.push([pred.id, pred.name, (pred.code ? pred.code : ""), pred.periodType, pred.generator.missingValueStrategy, pred.generator.description, pred.generator.expression, targetName, pred.outputCombo.id, pred.sequentialSampleCount, pred.annualSampleCount]);
		}
	...
	if (metaData.indicators && metaData.indicators.length > 0) {
		...
		tab = [["UID", "name", "shortName", "code", "description", "numerator", "numeratorDescription", "denominator", "denominatorDescription", "indicatorType", "decimals", "annualized", "indicatorGroup UIDs"]];
		...
		for (var i = 0; i < metaData.indicators.length; i++) {
			...
			tab.push([ind.id, ind.name, ind.shortName, (ind.code ? ind.code : ""), (ind.description ? ind.description : ""),
			ind.numerator, (ind.numeratorDescription ? ind.numeratorDescription : ""), ind.denominator, (ind.denominatorDescription ? ind.denominatorDescription : ""), type,
			(ind.decimals ? ind.decimals : ""), (ind.annualized ? ind.annualized : ""), indicatorGroupsFromIndicator(ind.id, metaData.indicatorGroups)]);
		}
	...
	if (metaData.dashboards && metaData.dashboards.length > 0) {
		...
		let xlDashTab = [["UID", "name", "code", "description"]];
		...
		for (var i = 0; i < metaData.dashboards.length; i++) {
			...
			tab = [["UID", "Name", "code", "description"]];
			tabTemp = ([db.id, db.name, (db.code ? db.code : ""), (db.description ? db.description : "")]);
			tab.push(tabTemp);
			...
			for (var j = 0; j < db.dashboardItems.length; j++) {
				...
				tabTemp = [id, type, (name ? name : ""), dbi.id, (dbi.lastUpdated ? dbi.lastUpdated.substr(0, 10) : ""), db.id];
				tab.push(tabTemp);
	...
	if (metaData.visualizations && metaData.visualizations.length > 0) {
		...
		tab = [["UID", "Name", "title", "subtitle", "type", "userOrganisationUnit", "description"]];

		for (let i = 0; i < metaData.visualizations.length; i++) {
			let item = metaData.visualizations[i];
			tab.push([item.id, (item.name ? item.name : ""), (item.title ? item.title : ""), (item.subtitle ? item.subtitle : ""), item.type, item.userOrganisationUnit, (item.description ? item.description : "")]);
		}
	...
	if (metaData.charts && metaData.charts.length > 0) {
		...
		for (var i = 0; i < metaData.charts.length; i++) {
			...
			tab.push([item.name, (item.description ? item.description : " "), (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), item.id]);
		}
				...
				for (var i = 0; i < metaData.maps.length; i++) {
					...
					for (var j = 0; j < item.mapViews.length; j++) {
						if (mv.id === item.mapViews[j].id) {
							tab.push([item.name, item.id, (mv.lastUpdated ? mv.lastUpdated.substr(0, 10) : ""), mv.id]);
						}
					}
				}
			}
	...
	if (metaData.legendSets && metaData.legendSets.length > 0) {
		...
		for (var i = 0; i < metaData.legendSets.length; i++) {
			...
			tab.push(["Last updated:", (legendSet.lastUpdated ? legendSet.lastUpdated.substr(0, 10) : "")]);
			...
			for (var j = 0; j < legendSet.legends.length; j++) {
				...
				tab.push([item.name, item.startValue, item.endValue, (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), item.id]);
				xlLegendTab.push([legendSet.name, legendSet.id, item.name, item.startValue, item.endValue, (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), item.id]);
			}
			content += utils.htmlTableFromArray(tab, true);
		}
	...
	if (metaData.trackedEntityInstanceFilters && metaData.trackedEntityInstanceFilters.length > 0) {
		...
		for (let i = 0; i < metaData.trackedEntityInstanceFilters.length; i++) {
			...
			tab.push([teif.name, (teif.lastUpdated ? teif.lastUpdated.substr(0, 10) : ""), teif.id]);
		}
	...
	if (metaData.programNotificationTemplates && metaData.programNotificationTemplates.length > 0) {
		...
		for (let i = 0; i < metaData.programNotificationTemplates.length; i++) {
			...
			tab.push([(item.name ? item.name : ""), item.id, (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), (item.subjectTemplate ? item.subjectTemplate : ""), (item.messageTemplate ? item.messageTemplate : "")]);
		}
	...
	if (metaData.users && metaData.users.length > 0) {
		...
		for (var i = 0; i < metaData.users.length; i++) {
			...
			let userName = "";
			if (item.userCredentials && item.userCredentials.username) {
				userName = item.userCredentials.username;
			}
			tab.push([userName, (item.lastUpdated ? item.lastUpdated.substr(0, 10) : ""), item.id]);
		}
